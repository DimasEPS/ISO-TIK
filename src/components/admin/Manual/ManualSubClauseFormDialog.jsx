...
const EMPTY_FORM = { nomor: "", judul: "" };

export function ManualSubClauseFormDialog({
  open,
  onOpenChange,
  mode = "create",
  parentTitle,
      ...
      <DialogContent className="w-full max-w-[560px] bg-white px-8 py-6 text-navy">
        <DialogHeader className="gap-1">
          <DialogTitle className="heading-4-bold">
            {isEdit ? "Edit Sub Klausa" : "Tambah Sub Klausa"}
          </DialogTitle>
          <p className="text-sm text-gray-dark">
            Lengkapi form di bawah ini untuk {isEdit ? "memperbarui" : "menambah"} Sub Klausa baru
          </p>
          {parentTitle && (
            <span className="mt-1 inline-flex w-fit items-center rounded-sm bg-gray-dark2 px-3 py-1 text-sm text-navy">
              {parentTitle}
            </span>
          )}
        </DialogHeader>
